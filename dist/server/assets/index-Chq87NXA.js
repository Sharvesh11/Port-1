import { a5 as InteractorType, ah as getDistance, ac as getRangeValue, b5 as RotateDirection, ae as getRandom, b6 as getDistances, am as clamp, b7 as getRangeMax, a1 as isObject, ab as degToRad, r as rangeColorToHsl, b8 as getHslAnimationFromHsl, b9 as updateColor, ba as hslToRgb, bb as hslaToRgba, bc as parseAlpha, ax as initParticleNumericAnimationValue, bd as percentDenominator, be as updateAnimation, _ as OutMode, bf as OutModeDirection, bg as calculateBounds, ar as ParticleOutType, bh as isPointInside, as as Vector, ad as randomInRange, bi as EasingType, az as isInArray, bj as loadFont, u as executeOnSingleOrMultiple, av as itemFromSingleOrMultiple, aC as Circle, P as isNull, w as millisecondsToSeconds, x as mouseMoveEvent, bk as divModeExecute, bl as circleBounce, bm as circleBounceDataFromParticle, aB as Rectangle, bn as rectBounce, R as DivType, bo as isDivModeEnabled, X as isArray, O as OptionsColor, F as mouseLeaveEvent, bp as rgbToHsl, bq as colorMix, br as divMode, bs as drawLine$1, h as getStyleFromHsl, g as getStyleFromRgb, bt as getLinkRandomColor, bu as getLinkColor, W as setRangeValue, U as deepExtend, bv as itemFromArray, n as getLogger, ag as errorPrefix, Y as ValueWithRandom, aP as isSsr, Z as CollisionMode, a as rangeColorToRgb, bw as AnimationStatus, a3 as DestroyType } from "./EasingType-Ws52dl2p.js";
...
const half$7 = 0.5, double$9 = 2, minVelocity$4 = 0, identity$4 = 1, moveSpeedFactor = 60, minSpinRadius = 0, spinFactor = 0.01, doublePI$2 = Math.PI * double$9;
...
function move(particle, moveOptions, moveSpeed, maxSpeed, moveDrift, reduceFactor, delta) {
  ...
  const gravityOptions = particle.gravity, gravityFactor = gravityOptions?.enable && gravityOptions.inverse ? -identity$4 : identity$4;
...
function drawEmoji(data, image) {
  const { context, opacity } = data, half2 = 0.5, previousAlpha = context.globalAlpha;
  if (!image) {
    return;
  }
...
class Bubbler extends ExternalInteractorBase {
  constructor(container, engine) {
    ...
    this._hoverBubbleOpacity = (particle, ratio, divBubble) => {
      ...
      const optOpacity = particle.options.opacity.value, pOpacity = particle.opacity?.value ?? defaultOpacity$2, opacity = calculateBubbleValue(pOpacity, modeOpacity, getRangeMax(optOpacity), ratio);
      if (opacity !== void 0) {
    ...
    this._singleSelectorHover = (delta, selector, div) => {
      ...
      selectors.forEach((item) => {
        ...
        }, repulseRadius = elem.offsetWidth * half$5 * pxRatio, area = div.type === DivType.circle ? new Circle(pos.x, pos.y, repulseRadius) : new Rectangle(elem.offsetLeft * pxRatio, elem.offsetTop * pxRatio, elem.offsetWidth * pxRatio, elem.offsetHeight * pxRatio), query = container2.particles.quadTree.query(area, (p) => this.isEnabled(p));
        for (const particle of query) {
...
function drawStar(data) {
  const { context, particle, radius } = data, sides2 = particle.sides, inset = particle.starInset ?? defaultInset$1;
